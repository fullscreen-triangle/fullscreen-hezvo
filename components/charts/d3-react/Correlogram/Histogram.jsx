...
export const Histogram = ({ width, height, data, limits }) => {
    ...
    const allRects = groupBuckets.map((group, i) => group.buckets.map((bucket, j) => (<rect key={i + "_" + j} x={xScale(bucket.x0) + BUCKET_PADDING / 2} width={xScale(bucket.x1) - xScale(bucket.x0) - BUCKET_PADDING} y={yScale(bucket.length)} height={boundsHeight - yScale(bucket.length)} fill={colorScale(group.group.group)} opacity={1}/>)));
