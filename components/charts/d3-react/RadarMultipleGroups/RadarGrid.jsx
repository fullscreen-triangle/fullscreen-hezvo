...
export const RadarGrid = ({ outerRadius, xScale, axisConfig, }) => {
    ...
    const allAxes = axisConfig.map((axis, i) => {
        ...
        <text x={labelPosition.x} y={labelPosition.y} fontSize={12} fill={GRID_COLOR} textAnchor={labelPosition.x > 0 ? 'start' : 'end'} dominantBaseline="middle">
          {axis.name}
        </text>
      </g>);
    });
    ...
    const allCircles = [...Array(GRID_NUMBER).keys()].map((position, i) => {
        return (<circle key={i} cx={0} cy={0} r={INNER_RADIUS +
                (position * (outerRadius - INNER_RADIUS)) / (GRID_NUMBER - 1)} stroke={GRID_COLOR} fill="none"/>);
    });
    return (<g>
      {allAxes}
      {allCircles}
