...
export const Scatterplot = ({ width, height, data }) => {
    ...
    const allShapes = data.map((d, i) => {
        const color = d.x === 50 && d.y === 50
            ? "blue"
            : d.x === 20 && d.y === 20
                ? "red"
                : "purple";
        return (<CircleItem key={d.name} x={xScale(d.x)} y={yScale(d.y)} color={color}/>);
    });
    return (<div>
...
const CircleItem = ({ x, y, color }) => {
    ...
    return (<animated.circle r={springProps.r} cx={springProps.x} cy={springProps.y} opacity={1} fill={color} fillOpacity={springProps.opacity}/>);
};
